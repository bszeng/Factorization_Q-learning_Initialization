...
def run_agent_QL(env, agent_tpye, exploration_approach, FM,  FM_freq, seed,plotting=False):
...
for FM_threshold in Approximation_threshold:    
    for seed in seeds:
        ...
        agent = QL(seed=seed)
        run_agent_QL(env, agent_tpye='QL', exploration_approach='boltzmann', FM=True, FM_freq = FM_threshold, seed=seed)
        
        random.seed(seed)
        np.random.seed(seed) 
        env = radio_environment(seed=seed)
        agent = QL(seed=seed)
        run_agent_QL(env, agent_tpye='QL', exploration_approach='boltzmann', FM=False, FM_freq = FM_threshold, seed=seed) 
        
        random.seed(seed)
        np.random.seed(seed)
        env = radio_environment(seed=seed)
        agent = QL(seed=seed)
        run_agent_QL(env, agent_tpye='QL', exploration_approach='egreedy', FM=False, FM_freq = FM_threshold, seed=seed) 

        random.seed(seed)
        np.random.seed(seed)
        env = radio_environment(seed=seed)
        agent = QL(seed=seed)
        run_agent_QL(env, agent_tpye='QL', exploration_approach='egreedy', FM=True, FM_freq = FM_threshold, seed=seed) 
